...
from PyQt6.QtCore import QSettings, Qt, QDateTime, QMutex
from PyQt6.QtSql import QSqlDatabase, QSqlTableModel, QSqlRecord, QSqlField, QSqlQuery

from threads.download_thread import DownloadThread
from tables_views.processed_view import ProcessedTableView
from tables_views.queue_view import QueueTableView
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.log_text.setTextColor(QtGui.QColor("green"))
        self.log_text.setStyleSheet("background-color: black;")
        log_layout.addWidget(self.log_text)

        # Message window
        self.messages = LogTextEdit()
        self.messages.setReadOnly(True)
        font = QtGui.QFont("Courier New", 10)
        self.messages.setFont(font)
        self.messages.setTextColor(QtGui.QColor("white"))
        self.messages.setStyleSheet("background-color: black;")
        ...
        self.processed_table.setSelectionBehavior(QAbstractItemView.SelectionBehavior.SelectRows)
        self.processed_table.setSelectionMode(QAbstractItemView.SelectionMode.ExtendedSelection)
    ...
    def add_queue(self):
        ...
        if url:
            ...
            if count > 0:
                ...
                self.messages.setTextColor(yellow_color)
                self.messages.append("URL already exists in the database")
            else:
                record = QSqlRecord()
                ...
                self.messages.setTextColor(white_color)
                self.messages.append("URL added to queue")
    ...
    def start(self):
        ...
        if self.downloading == True:
            ...
            c.execute('SELECT id, url FROM urls WHERE processed = 0 ORDER BY id LIMIT 1')
            row = c.fetchone()
            ...
            if row:
                ...
                self.messages.setTextColor(white_color)
                self.messages.append(f"Starting download of URL: {url}")
                ...
                self.download_thread.finished.connect(lambda: self.download_finished(conn, c))
    ...
    def download_finished(self, conn, c):
        ...
        if self.aborted == True:
            ...
            self.messages.setTextColor(red_color)
            self.messages.append("Download process aborted.")
        else:
            ...
            self.queue_model.setData(
                self.queue_model.index(record_index.row(), self.queue_model.fieldIndex('processed')),
                1
            )
            self.queue_model.setData(
            ...
            self.messages.setTextColor(green_color)
            self.messages.append(f"Finished download of URL: {url}")
            ...
            if row:
                ...
                conn.close()
                self.messages.setTextColor(green_color)
                self.messages.append("No more urls to download, queue finished.")
                self.set_button_start()
                self.downloading = False 
        # Release the mutex after writing to the database
        db_mutex.unlock()              
